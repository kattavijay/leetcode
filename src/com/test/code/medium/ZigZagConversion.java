...
System.out.println(conversion(name,3));
	...
	private static String conversion(String s,int numRows) {
		 char[] str = s.toCharArray();
	        ...
	        for (int i = 0; i < sBuffer.length; i++) sBuffer[i] = new StringBuffer();
	        int j = 1;
	        for(int i = 0; i < str.length; i ++){
	            if(j > numRows) sBuffer[2 * numRows - j].append(str[i]);
	            else sBuffer[j].append(str[i]);
	            if(j == 2 * numRows - 2 || numRows == 1) j = 1;
	            else j ++;
	        }
